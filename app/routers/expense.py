...
def create_expense(expense: ExpenseCreate, db: Session = Depends(get_db), manager = Depends(verify_token)):
    return expense_service.create_expense(db, expense, manager)

@router.get("/{expense_id}", response_model=ExpenseResponse)
def get_expense(expense_id: int, db: Session = Depends(get_db),  manager= Depends(verify_token)):
    return expense_service.get_expense(db, expense_id, manager)


@router.get("/", response_model=list[ExpenseResponse])
def get_expenses(db: Session = Depends(get_db), manager= Depends(verify_token)):
